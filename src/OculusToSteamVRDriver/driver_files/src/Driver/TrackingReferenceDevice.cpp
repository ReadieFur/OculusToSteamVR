...
OculusToSteamVRDriver::TrackingReferenceDevice::TrackingReferenceDevice(std::string serial):
    serial_(serial)
{
...
std::string OculusToSteamVRDriver::TrackingReferenceDevice::GetSerial()
{
    return this->serial_;
}

void OculusToSteamVRDriver::TrackingReferenceDevice::Update()
{
...
DeviceType OculusToSteamVRDriver::TrackingReferenceDevice::GetDeviceType()
{
    return DeviceType::TRACKING_REFERENCE;
}

vr::TrackedDeviceIndex_t OculusToSteamVRDriver::TrackingReferenceDevice::GetDeviceIndex()
{
    return this->device_index_;
}

vr::EVRInitError OculusToSteamVRDriver::TrackingReferenceDevice::Activate(uint32_t unObjectId)
{
...
void OculusToSteamVRDriver::TrackingReferenceDevice::Deactivate()
{
    this->device_index_ = vr::k_unTrackedDeviceIndexInvalid;
}

void OculusToSteamVRDriver::TrackingReferenceDevice::EnterStandby()
{
}

void* OculusToSteamVRDriver::TrackingReferenceDevice::GetComponent(const char* pchComponentNameAndVersion)
{
    return nullptr;
}

void OculusToSteamVRDriver::TrackingReferenceDevice::DebugRequest(const char* pchRequest, char* pchResponseBuffer, uint32_t unResponseBufferSize)
{
    if (unResponseBufferSize >= 1)
        pchResponseBuffer[0] = 0;
}

vr::DriverPose_t OculusToSteamVRDriver::TrackingReferenceDevice::GetPose()
{
    return last_pose_;
}
